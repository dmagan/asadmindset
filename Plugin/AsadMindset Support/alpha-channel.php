...
class Alpha_Channel {
    ...
    'replyToId' => $post->reply_to_id ? (int) $post->reply_to_id : null
        ...
        // Send push notifications to all users (respects per-user preferences)
        $this->send_push_to_all_users(
            '📢 کانال آلفا',
            mb_substr($post->content, 0, 100) . (mb_strlen($post->content) > 100 ? '...' : ''),
            array('type' => 'alpha', 'postId' => (string) $post_id)
        );
   ...
   /**
    ...
    /**
     * Send push notification to users with active alpha subscription + team members
     * Uses Push_Notifications class which checks per-user preferences
     */
    private function send_push_to_all_users($title, $body, $data = array()) {
        global $wpdb;
        
        // Ensure Push_Notifications class is available
        if (!class_exists('AsadMindset_Push_Notifications')) {
            $push_file = plugin_dir_path(__FILE__) . 'push-notifications.php';
        ...
        if (!class_exists('AsadMindset_Push_Notifications')) {
            error_log('ALPHA PUSH: AsadMindset_Push_Notifications class not found');
            return;
        }
        ...
        error_log('ALPHA PUSH: Subscribed users: ' . implode(',', $sub_users));
        
        // Get active sub-admins
        $team_users = $wpdb->get_col(
            "SELECT DISTINCT user_id FROM $table_sa WHERE is_active = 1"
        );
        error_log('ALPHA PUSH: Team users: ' . implode(',', $team_users));
        
        // Get main admin(s)
        $admins = get_users(array('role' => 'administrator', 'fields' => 'ID'));
        error_log('ALPHA PUSH: Admins: ' . implode(',', $admins));
        
        // Merge all unique user IDs
        $all_user_ids = array_unique(array_merge(
            array_map('intval', $sub_users),
            array_map('intval', $team_users),
            array_map('intval', $admins)
        ));
        
        error_log('ALPHA PUSH: Total unique users: ' . count($all_user_ids) . ' - IDs: ' . implode(',', $all_user_ids));
        
        foreach ($all_user_ids as $user_id) {
            $result = $push->send_to_user($user_id, $title, $body, $data);
            error_log('ALPHA PUSH: User ' . $user_id . ' result: ' . json_encode($result));
        }
    }
}
