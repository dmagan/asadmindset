...
const AdminChatView = ({ conversationId, onBack }) => {
...
useEffect(() => {
  ...
  useEffect(() => {
    if (conversationId && messages.length > 0) {
      const unreadUserMessages = messages.filter(m => m.sender === 'support' && m.status !== 'read');
  ...
  const loadConversation = async () => {
    try {
      ...
      // Set conversation info for header
      setConversation(data.conversation);
      
      // For admin view: admin messages = user (right side), user messages = support (left side)
      const formattedMessages = data.messages.map(msg => ({
        ...
        replyTo: msg.replyTo ? {
          ...
          sender: msg.replyTo.sender === 'admin' ? 'user' : 'support'
        } : null
      }));
    ...
    channelRef.current.bind('new-message', (data) => {
      if (data.sender === 'user') {
        const newMsg = {
          ...
          sender: 'support', // user messages appear on left (support side)
  ...
  const sendTypingIndicator = async (isTyping) => {
    ...
    try {
      ...
      await fetch(`${API_URL}/admin/conversations/${conversationId}/typing`, {
        method: 'POST',
        headers: {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      const messageResponse = await fetch(`${API_URL}/admin/conversations/${conversationId}/message`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
